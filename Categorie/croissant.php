      ...
      <div class="first">
        <div class="centre">
          ...
          <div class="pub">
            ...
            <p>
              "Venez savourer notre croissant artisanal,<br />
              préparé avec amour et expertise chaque matin.<br />
              Un régal pour les papilles qui vous transportera immédiatement<br />
              en France !"
            </p>
          </div>
        </div>
...
try {
    ...
    if ($tri === 'Prix: faible à élevé') {
        $query = "SELECT * FROM article WHERE catArt = 1 ORDER BY prixArt ASC";
    } elseif ($tri === 'Prix: élevé à faible') {
        $query = "SELECT * FROM article WHERE catArt = 1 ORDER BY prixArt DESC";
    } else {
        $query = "SELECT * FROM article WHERE catArt = 1";
    }
